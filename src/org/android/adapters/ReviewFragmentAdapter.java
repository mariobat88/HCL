...
public class ReviewFragmentAdapter extends BaseAdapter{
	...
	public ReviewFragmentAdapter(List<Item> items, Context context) {
		this.items = items;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(view == null){
			...
			view = inflater.inflate(R.layout.fragment_review_list_item, null);
		}
		

		ProgressBar progress = (ProgressBar) view.findViewById(R.id.review_list_item_progress);
		progress.setIndeterminate(true);
		
		HCLImageView image= (HCLImageView) view.findViewById(R.id.review_list_item_image);
		Util.DownloadImage(context, item.getImage(), image, progress);

		Spanned descriptionText = Html.fromHtml(item.getDescription());
		
		TextView description = (TextView) view.findViewById(R.id.review_list_item_description);
